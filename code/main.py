...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        checkbox4_label = "Juggernaut-XL_v9(6.41GB)"
        ...
        if os.path.exists(folder_path) and os.path.isdir(folder_path):
            ...
            checkbox1_label += " ×フォルダーが存在しません"
            checkbox2_label += " ×フォルダーが存在しません"
            checkbox3_label += " ×フォルダーが存在しません"
            checkbox4_label += " ×フォルダーが存在しません"
        ...
        if os.path.exists(folder_path) and os.path.isdir(folder_path):
            ...
            print(f"指定されたフォルダー {folder_path} は存在しません。")
            canny_label += " ×フォルダーが存在しません"
            depth_label += " ×フォルダーが存在しません"
            openpose_label += " ×フォルダーが存在しません"
            softedge_label += " ×フォルダーが存在しません"
    ...
    def start_download(self):
        ...
        if self.checkbox1.isChecked():
            if not os.path.exists("momoiropony_v14.safetensors"):
                ...
                subprocess.run(["curl", "-L", "https://huggingface.co/kataragi/AIGen/resolve/main/momoiropony_v14.safetensors", "-o", "momoiropony_v14.safetensors", "--progress-bar"], check=True)
            # 二階層上のディレクトリを取得
            ...
            # 所定のテキスト
            text_to_append = "momoiropony_v14,\"japanese woman, (natural lighting), \",\"(cleavage:2), (illustration), 3d, 2d, painting, cartoons, sketch, watercolor, monotone,(kimono), (crossed eyes), (strabismus)\""
            try:
                # styles.csvファイルを読み込む
        ...
        if self.checkbox2.isChecked():
            if not os.path.exists("animagine-xl-3.1.safetensors"):
                ...
                Ssubprocess.run(["curl", "-L", "https://huggingface.co/kataragi/AIGen/resolve/main/animagine-xl-3.1.safetensors", "-o", "animagine-xl-3.1.safetensors", "--progress-bar"], check=True)
            # 二階層上のディレクトリを取得
            current_dir = os.getcwd()
            parent_dir = os.path.dirname(current_dir)
            grandparent_dir = os.path.dirname(parent_dir)
        ...
        if self.checkbox3.isChecked():
            if not os.path.exists("fuduki_mix_v20.safetensors"):
                ...
                subprocess.run(["curl", "-L", "https://huggingface.co/kataragi/AIGen/resolve/main/fuduki_mix_v20.safetensors", "-o", "fuduki_mix_v20.safetensors", "--progress-bar"], check=True)
            # 二階層上のディレクトリを取得
            current_dir = os.getcwd()
            parent_dir = os.path.dirname(current_dir)
            grandparent_dir = os.path.dirname(parent_dir)
        ...
        if self.checkbox4.isChecked():
            if not os.path.exists("Juggernaut-XL_v9_RunDiffusionPhoto_v2.safetensors"):
                ...
                subprocess.run(["curl", "-L", "https://huggingface.co/kataragi/AIGen/resolve/main/Juggernaut-XL_v9_RunDiffusionPhoto_v2.safetensors", "-o", "Juggernaut-XL_v9_RunDiffusionPhoto_v2.safetensors", "--progress-bar"], check=True)
            # 二階層上のディレクトリを取得
            current_dir = os.getcwd()
            parent_dir = os.path.dirname(current_dir)
            grandparent_dir = os.path.dirname(parent_dir)
        ...
        if self.checkbox_canny.isChecked():
            ...
            if not os.path.exists(destination):
                ...
                file_url = "https://huggingface.co/lllyasviel/sd_control_collection/resolve/main/diffusers_xl_canny_full.safetensors"
        ...
        if self.checkbox_depth.isChecked():
            ...
            if not os.path.exists(destination):
                ...
                file_url = "https://huggingface.co/lllyasviel/sd_control_collection/resolve/main/diffusers_xl_depth_full.safetensors"
                download_file(file_url, destination) # ファイルをダウンロードするために関数を呼び出します
                print(f"{destination}にファイルをダウンロードしました。")
            else:
        ...
        if self.checkbox_openpose.isChecked():
            ...
            if not os.path.exists(destination):
                ...
                file_url = "https://huggingface.co/lllyasviel/sd_control_collection/resolve/main/thibaud_xl_openpose.safetensors"
                download_file(file_url, destination) # ファイルをダウンロードするために関数を呼び出します
                print(f"{destination}にファイルをダウンロードしました。")
            else:
        ...
        if self.checkbox_softedge.isChecked():
            ...
            if not os.path.exists(destination):
                ...
                file_url = "https://huggingface.co/lllyasviel/sd_control_collection/resolve/main/sargezt_xl_softedge.safetensors"
                download_file(file_url, destination) # ファイルをダウンロードするために関数を呼び出します
                print(f"{destination}にファイルをダウンロードしました。")
            else:
